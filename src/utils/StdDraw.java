...
public final class StdDraw implements ActionListener, MouseListener, MouseMotionListener, KeyListener {
	...
	private static DrawGraph gGui;
	public static void setGui(DrawGraph g) {
		gGui = g;
	}
	...
	private double path(StringBuilder sb) {
		Collection<node_data> points = gGui.getGraph().getV();
		...
		if(gGui.getAlgo().shortestPath(src, dest)==null)
			JOptionPane.showMessageDialog(null, "The isn't a path between those two nodes","Messege",0);

		else {
			ArrayList<node_data> lista = (ArrayList<node_data>) gGui.getAlgo().shortestPath(src, dest);
		...
		return (gGui.getAlgo().shortestPathDist(src, dest));
	...
	public void actionPerformed(ActionEvent e) {
		...
		switch (e.getActionCommand()) {
			...
			case "isConnected":
				if(gGui.getGraph().nodeSize()==0) break;
				boolean b = (gGui.getAlgo().isConnected());
				if (b==true)
			...
			case "TSP":
				...
				Collection<node_data> tspNodes = gGui.getGraph().getV();
				...
				List<node_data> tspList =  gGui.getAlgo().TSP(targets);
				...
				if(tspList!=null) {
					for (int m = 0; m < tspList.size() - 1; m++) {
						...
						str.append(tspList.get(m).getKey()+"->");
					}
					str.append(tspList.get(tspList.size()-1)+"");

					JOptionPane.showMessageDialog(null, "TSP: " + str.toString());
			...
			case "connect":
				...
				Collection<node_data> nodes = gGui.getGraph().getV();
				...
				try {
					gGui.getGraph().connect(src, dest, Double.parseDouble(edgeWeight));
					gGui.Edges();
					gGui.Edges();
